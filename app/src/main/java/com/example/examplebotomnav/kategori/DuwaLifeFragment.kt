...
import com.example.examplebotomnav.databinding.FragmentDuwaLifeBinding
...
class DuwaLifeFragment : Fragment() {
    private lateinit var adapter: Adapter
    private val list = ArrayList<ResultsItem>()

    private var _binding: FragmentDuwaLifeBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentDuwaLifeBinding.inflate(inflater, container, false)
        val root: View = binding.root

        adapter = context?.let { Adapter(list, it) }!!

        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvLife.layoutManager = layoutManager
        binding.rvLife.adapter = adapter
